...
CREATE TABLE `node` (
  ...
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT '主机初始化状态，0:未初始化，1:已初始化',
...
CREATE TABLE `cluster` (
  ...
  `enable_sentinel` VARCHAR(7) DEFAULT NULL COMMENT '是否开启sentinel监控',
...
CREATE TABLE `instance` (
  ...
  `redis_version` VARCHAR(16) NOT NULL,
...
CREATE TABLE `application_form` (
  ...
  `redis_mode` VARCHAR(16) NOT NULL COMMENT '集群模式',
  `redis_version` VARCHAR(16) NOT NULL COMMENT 'redis版本',
  `cache_size` SMALLINT NOT NULL COMMENT '内存大小',
  `master_count` SMALLINT NOT NULL COMMENT '主节点数量',
  `replica_count` TINYINT NOT NULL COMMENT '备份节点数量',
...
CREATE TABLE `config_dict` (
  `id` INT NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `redis_mode` VARCHAR(16) NOT NULL COMMENT '集群模式,single or cluster',
  `redis_version` VARCHAR(16) NOT NULL COMMENT 'redis版本',
...
CREATE TABLE `cluster_scale` (
  ...
  `result_desc` VARCHAR(512) DEFAULT NULL COMMENT '结果备注',
